...
public:
	...
	void Initialize(TextureType* pTexture, Vector2 position, float rotinDegrees, float scale, Vector2 moveSpeed, float rotationSpeed, Color color, float layer);
	...
	// get/set the speed
	Vector2 GetMoveSpeed() const { return moveSpeed; }
	void SetMoveSpeed(Vector2 f) { moveSpeed = f; }

	// get/set the speed
	float GetSpinSpeed() const { return spinSpeed; }
	void SetSpinSpeed(float f) { spinSpeed = f; }
